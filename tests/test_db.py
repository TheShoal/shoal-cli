...
async def db(tmp_path):
    ...
    await db.initialize()
    yield db
    # No explicit close needed if we use connection pooling or context managers inside ShoalDB
...
async def test_db_initialization(tmp_path):
    ...
    async with aiosqlite.connect(db_path) as conn:
        async with conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='sessions'") as cursor:
