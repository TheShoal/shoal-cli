...
class TestMcpMatrixContext:
    def test_empty_sessions(self) -> None:
        ctx = mcp_matrix_context(sessions=[], available_servers=["a", "b"], groups=[])
        assert ctx["sessions"] == []
    ...
    def test_sessions_sorted_by_name(self) -> None:
        ...
        ctx = mcp_matrix_context(sessions=sessions, available_servers=[], groups=[])
        rows = ctx["sessions"]
    ...
    def test_mcp_enabled_mapping(self) -> None:
        session = _make_session(mcp_servers=["memory"])
        ctx = mcp_matrix_context(
            sessions=[session], available_servers=["memory", "fs"], groups=[]
        )
        row = ctx["sessions"][0]
        assert row["mcp_enabled"] == {"memory": True, "fs": False}

    def test_stopped_session_is_stopped(self) -> None:
        session = _make_session(status=SessionStatus.stopped)
        ctx = mcp_matrix_context(sessions=[session], available_servers=[], groups=[])
    ...
    def test_groups_passed_through(self) -> None:
        ...
        ctx = mcp_matrix_context(sessions=[], available_servers=[], groups=groups)
