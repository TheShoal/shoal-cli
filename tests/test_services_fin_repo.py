...
def test_registry_url():
    # test latest
    assert registry_url("fin:foo", "https://fins.shoal.dev") == "https://fins.shoal.dev/foo/latest.tar.gz"
    # test versioned
    assert registry_url("fin:foo@1.0.0", "https://fins.shoal.dev") == "https://fins.shoal.dev/foo/1.0.0.tar.gz"
    # test empty version
    assert registry_url("fin:foo@", "https://fins.shoal.dev") == "https://fins.shoal.dev/foo/latest.tar.gz"
...
def test_download_fin_invalid_extension():
    ...
    with pytest.raises(ValueError, match="Unsupported archive extension for fin download.*"):
        download_fin(url)
...
def test_download_fin_http_error(mock_stream, tmp_path):
    ...
    with pytest.raises(ValueError, match="Failed to download fin: HTTP 404 from.*"):
        download_fin(url)
