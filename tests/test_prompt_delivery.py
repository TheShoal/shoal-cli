...
from shoal.core.prompt_delivery import build_tool_command_with_prompt, write_prompt_file
...
def test_build_keys_mode_returns_base_command() -> None:
    """keys mode returns the base command unchanged, ignoring the prompt."""
    tool = _make_tool(command="pi", input_mode="keys")
    assert build_tool_command_with_prompt(tool, "some prompt", "sess-1") == "pi"
...
def test_build_flag_mode_opencode() -> None:
    ...
    assert (
        build_tool_command_with_prompt(tool, "Fix the bug", "sess-2")
...
def test_build_flag_mode_default_flag() -> None:
    ...
    assert build_tool_command_with_prompt(tool, "do work", "sess-3") == "mytool --prompt 'do work'"


def test_build_flag_mode_quotes_special_chars() -> None:
    """flag mode shell-quotes prompts with spaces and special chars."""
    tool = _make_tool(command="opencode", input_mode="flag", prompt_flag="--prompt")
    result = build_tool_command_with_prompt(tool, "Say 'hello' & goodbye", "sess-4")
    assert result.startswith("opencode --prompt ")
    assert shlex.split(result) == ["opencode", "--prompt", "Say 'hello' & goodbye"]
...
def test_build_arg_mode_no_prefix_claude() -> None:
    ...
    assert (
        build_tool_command_with_prompt(tool, "Refactor the module", "sess-5")
        == "claude 'Refactor the module'"
    )
...
def test_build_arg_mode_no_prefix_quotes_special_chars() -> None:
    ...
    assert shlex.split(result) == ["claude", "it's tricky & fun"]
...
def test_build_arg_mode_with_prefix_writes_prompt_file(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "sess-8.md").read_text(encoding="utf-8") == "multi\nline\nprompt"
...
def test_build_arg_mode_with_prefix_no_space_between_prefix_and_path(tmp_path: Path) -> None:
    ...
    assert result.split(" ")[1].startswith("@/")


# ---------------------------------------------------------------------------
# Round-trip: shell-sensitive prompt text is safely quoted in arg/flag modes
# ---------------------------------------------------------------------------
...
def test_prompt_does_not_appear_raw_in_command(input_mode: str, flag: str, prefix: str) -> None:
    """Shell-sensitive prompt chars appear verbatim in one quoted token, not spread raw."""
    tool = _make_tool(
