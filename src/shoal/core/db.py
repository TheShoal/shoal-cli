...
class ShoalDB:
    ...
    async def _initialize_schema(self) -> None:
        ...
        """)
        # Claw scheduler: task queue for background jobs and agent-scheduled work.
        await self._conn.execute("""
        ...
        """)
        await self._conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_claw_tasks_run_at
            ON claw_tasks(status, run_at)
        """)
        await self._conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_claw_tasks_session
            ON claw_tasks(session, status)
        """)
    ...
    async def create_claw_task(
        ...
        *,
        ...
        """Insert a new claw task and return its ID."""
        from datetime import UTC, datetime
        ...
        async with self._connection() as conn:
            cursor = await conn.execute(
                ...
                " (session, task_type, name, handler, cron_expr, interval_seconds,"
                " payload_json, run_at, max_retries, correlation_id, created_at, metadata_json)"
                " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    session,
                    task_type,
    ...
    async def get_claw_task(self, task_id: int) -> dict[str, object] | None:
        """Return a claw task by ID, or None."""
        async with self._connection() as conn:
            cursor = await conn.execute(
                "SELECT id, session, task_type, name, handler, cron_expr,"
                " interval_seconds, payload_json, run_at, status, retry_count,"
                " max_retries, correlation_id, created_at, last_run_at,"
                " completed_at, error, metadata_json"
                " FROM claw_tasks WHERE id = ?",
                (task_id,),
            )
            row = await cursor.fetchone()
        if row is None:
            return None
        return _row_to_claw_task(row)

    async def list_due_tasks(self, now: str, *, limit: int = 10) -> list[dict[str, object]]:
        """Return pending tasks whose run_at <= *now*, ordered by run_at."""
        async with self._connection() as conn:
            cursor = await conn.execute(
                "SELECT id, session, task_type, name, handler, cron_expr,"
                " interval_seconds, payload_json, run_at, status, retry_count,"
                " max_retries, correlation_id, created_at, last_run_at,"
                " completed_at, error, metadata_json"
                " FROM claw_tasks WHERE status = 'pending' AND run_at <= ?"
                " ORDER BY run_at LIMIT ?",
                (now, limit),
            )
            rows = await cursor.fetchall()
        return [_row_to_claw_task(r) for r in rows]

    async def list_session_tasks(
        self, session: str | None = None, *, status: str | None = None
    ) -> list[dict[str, object]]:
        """Return tasks for a session (or system tasks when session is None)."""
        clauses: list[str] = []
        params: list[object] = []
        if session is not None:
            ...
            clauses.append("session IS NULL")
        if status is not None:
        ...
        async with self._connection() as conn:
            cursor = await conn.execute(
                "SELECT id, session, task_type, name, handler, cron_expr,"  # noqa: S608
                " interval_seconds, payload_json, run_at, status, retry_count,"
                " max_retries, correlation_id, created_at, last_run_at,"
                " completed_at, error, metadata_json"
                f" FROM claw_tasks WHERE {where}"
                " ORDER BY run_at",
                params,
            )
            rows = await cursor.fetchall()
    ...
    async def claim_claw_task(self, task_id: int) -> bool:
        """Atomically move a task from pending to running (CAS)."""
        from datetime import UTC, datetime

        now = datetime.now(UTC).isoformat()
        async with self._connection() as conn:
            cursor = await conn.execute(
                "UPDATE claw_tasks SET status = 'running', last_run_at = ?"
    ...
    async def complete_claw_task(self, task_id: int) -> None:
        ...
        async with self._connection() as conn:
            await conn.execute(
                "UPDATE claw_tasks SET status = 'done', completed_at = ?"
    ...
    async def fail_claw_task(
        ...
        exhausted; otherwise sets a retry backoff and resets to pending.
        """
        from datetime import UTC, datetime, timedelta

        task = await self.get_claw_task(task_id)
        if task is None:
            return
        ...
        async with self._connection() as conn:
            await conn.execute(
                "UPDATE claw_tasks SET status = ?, retry_count = ?, error = ?,"
                " run_at = ? WHERE id = ?",
                (new_status, retry_count, error, run_at, task_id),
            )
            await conn.commit()

    async def cancel_claw_task(self, task_id: int) -> bool:
        """Cancel a pending task. Returns True if the task was cancelled."""
        async with self._connection() as conn:
            cursor = await conn.execute(
                "UPDATE claw_tasks SET status = 'cancelled' WHERE id = ? AND status = 'pending'",
                (task_id,),
            )
            await conn.commit()
        return (cursor.rowcount or 0) > 0

    async def reschedule_recurring_task(self, task_id: int) -> int | None:
        """Compute the next run_at for a recurring/cron task and insert a new pending row.

        Returns the new task ID, or None if the task is not recurring/cron.
        """
        task = await self.get_claw_task(task_id)
        if task is None:
            return None
        ...
        return await self.create_claw_task(
            name=str(task["name"]),
            handler=str(task["handler"]),
            run_at=next_run,
            session=task["session"],  # type: ignore[arg-type]
            task_type=task_type,
...
def _row_to_claw_task(row: Any) -> dict[str, object]:
    """Convert a DB row tuple to a claw task dict."""
    return {
        "id": row[0],
        "session": row[1],
        "task_type": row[2],
