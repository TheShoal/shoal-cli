...
Reference for shoal session lifecycle, pane identity, status detection guarantees, and cleanup protocol.
...
Consequences:
- `idle` can false-positive if the agent prints a prompt-like string mid-task. Trust `idle` only after `running` patterns have been absent for two consecutive poll cycles (~10 seconds).
...
If `auto_commit = true` is set in `config.toml`, shoal automatically creates a conventional commit on the working branch before executing `kill`.
...
Memory is NOT shared between agents across connections. If cross-agent state sharing is required, it must be persisted externally — files, SQLite, or a shared service.
